                    ...
                    <section class="checkout-block">
                        ...
                        <div class="checkout-delivery">
                            ...
                            <label class="checkout-delivery__option">
                                <input type="radio" name="delivery_method" value="{{ $slug }}"
                                    ...
                                    {{ old('delivery_method', $deliveryChoice['method'] ?? 'cdek') === $slug ? 'checked' : '' }}
                                    required>
                        ...
                        <div class="checkout-fields checkout-fields--2col" style="margin-top: 20px;">
                            <label class="checkout-field">
                                ...
                                $cityValue = $isPickup
                                ? 'Казань'
                                : old('delivery_city', $defaults['delivery_city'] ?? '');
                                @endphp
                                <input type="text" name="delivery_city" maxlength="80"
                                    value="{{ $cityValue }}"
                                    placeholder="Казань"
                                    {{ $isPickup ? 'readonly' : '' }}
                                    style="{{ $isPickup ? 'background: var(--bg-warm); opacity: 0.7; cursor: not-allowed;' : '' }}"
                                    required>
                                @if($isPickup)
                                <span class="checkout-field__hint">самовывоз только в Казани</span>
                                @endif
                                @error('delivery_city')
                                <span class="checkout-field__error">{{ $message }}</span>
                                @enderror
                            ...
                            <label class="checkout-field checkout-field--wide">
                                ...
                                <textarea name="delivery_note" maxlength="500" rows="2"
                                    placeholder="Код домофона, как лучше передать, особые пожелания…">{{ old('delivery_note', $defaults['delivery_note'] ?? '') }}</textarea>
                            </label>
                        </div>
                    </section>
